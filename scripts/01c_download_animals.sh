#!/bin/bash
# =============================================================================
# 01c_download_animals.sh
# Downloads a subset of the Animals-10 dataset from Kaggle, validates
# the images with ImageMagick, and merges them into the shared
# data/clean/ directory with the 'animal_' prefix.
#
# Same pattern as 01_download_clean.sh and 01b_download_flowers.sh —
# see those files for detailed comments on each step.
#
# Usage:
#   bash scripts/01c_download_animals.sh [--subset N]
#
# Options:
#   --subset N    Keep only N images (default: 5000)
#
# Outputs:
#   data/raw_animals/  — downloaded zip and extracted images
#   data/clean/        — validated images (prefixed 'animal_')
# =============================================================================
...
DATASET="alessiocorrado99/animals10"       # Kaggle dataset identifier
SUBSET_SIZE=5000                           # default images to keep
RAW_DIR="$SHARED_DIR/raw_animals"          # source-specific raw dir
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        *)
            ...
            echo "Usage: bash scripts/01c_download_animals.sh [--subset N]"
            exit 1
            ;;
    esac
done
...
if [[ ! -f "pipeline.sh" ]]; then
    ...
    echo "        e.g.:  bash scripts/01c_download_animals.sh"
    exit 1
fi
...
if [[ -f "$DOWNLOAD_STAMP" && -f "$ZIP_FILE" ]]; then
    ...
    echo "[04] Downloading Animals-10 archive from Kaggle..."
...
if [[ -f "$EXTRACT_STAMP" ]]; then
    ...
    FILE_LIST="$LOG_DIR/animals_extract_list.txt"
...
valid=0
skipped=0
LOG_FILE="$LOG_DIR/01c_animals_validation.log"

{
    echo "Animals validation log — target subset: $SUBSET_SIZE"
...
ANIMALS_IN_CLEAN=$(find "$CLEAN_DIR" -maxdepth 1 -iname "${FILE_PREFIX}*" | wc -l)

echo ""
echo "============================================================"
echo "  01c_download_animals.sh complete"
echo "------------------------------------------------------------"
echo "  Raw animals       : $RAW_DIR/    ($TOTAL_RAW total)"
echo "  Animals in clean/ : $ANIMALS_IN_CLEAN (prefixed '${FILE_PREFIX}')"
echo "  Clean total       : $CLEAN_DIR/  ($TOTAL_CLEAN images from all sources)"
echo "  Log               : $LOG_FILE"
echo "============================================================"
echo ""
echo "Next step: bash scripts/02_build_thumbnails.sh"
